...
// Enable CORS for all routes (necessary for public Shopify store widget script and API access)
...
try {
  ...
  console.error("Warning: Could not create or verify database directory:", err);
}

// Logger middleware to capture internal errors
app.use((req, res, next) => {
  const logFile = path.join(DB_DIR, "server_logs.txt");
...
app.get("/widget.js", (req, res) => {
  ...
  // Dynamically resolve SaaS server URL so it works both locally and on external live stores like Shopify
  const proto = req.headers["x-forwarded-proto"] || req.protocol;
  const host = req.headers["x-forwarded-host"] || req.get("host") || req.headers.host;
  const saasUrl = `${proto}://${host}`;
